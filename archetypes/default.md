...
# 最后修改时间，用于排序/显示更新时间
lastmod: {{ .Date }}
# 发布时间（独立于 date，未设置时回退到 date）
# publishDate: {{ .Date }}
# 过期时间，过期后文章不发布
# expiryDate: 2030-01-01T00:00:00+08:00
# 列表排序权重，数值越小越靠前（可用于置顶）
# weight: 1
# 自定义 URL 末尾段（默认用文件名）
# slug: custom-slug
# 完整 URL 覆盖（极少用）
...
# 标记为中日韩语言，影响摘要算法
# isCJKLanguage: true
# 链接用的标题（与 title 不同时使用）
# linkTitle: 链接显示的标题

# ==================== reimu 主题字段 ====================
# 文章封面图路径（用于列表卡片和文章顶部）
# cover: images/cover/cover2.webp
# 图库图片列表（article 中按 gallery 渲染）
# photos:
#   - https://example.com/img/1.webp
#   - https://example.com/img/2.webp
# 启用 KaTeX 数学公式渲染
math: false
# 启用 Mermaid 流程图渲染
mermaid: false
# 自定义摘要，覆盖自动生成的 summary（不填则用 .Summary）
# excerpt: "自定义摘要文本"
# 文章跳转到外部链接（卡片直接跳外链，不进文章页）
# link: https://example.com
# 标记文章过时（article 顶部显示提示）
# outdated: false
# 是否显示版权声明（默认遵循站点配置）
# copyright: true
# 是否显示赞助二维码
